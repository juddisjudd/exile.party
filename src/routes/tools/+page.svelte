<script lang="ts">
	...
	import { PRICING_LABEL } from '$lib/catalog/display';
	import {
		EMPTY_FILTERS,
		activeChipCount,
		filterTools,
	...
	import type { Pricing } from '$lib/catalog/schema';
	import { submitDialog } from '$lib/submit.svelte';

	let { data } = $props();

	const categoryIds = $derived(data.catalog.categories.map((c) => c.id));
	...
	const hasOfficial = $derived(data.catalog.tools.some((t) => t.official));
	...
	onMount(() => {
		filters = fromSearchParams(new URL(location.href).searchParams, categoryIds);
		synced = true;
	});
	...
	const categoryOrder = $derived(new Map(data.catalog.categories.map((c, i) => [c.id, i])));

	/* Sorted by category, then by name within each one. */
	const visible = $derived(
		filterTools(data.catalog.tools, filters).sort(
			(a, b) =>
	...
	function countFor(over: Partial<Filters>): number {
		return filterTools(data.catalog.tools, { ...filters, ...over }).length;
	}
...
<Meta
	title="Tools · exile.party"
	description="Browse third-party Path of Exile 1 and 2 tools by category, platform, price, and whether the source is open."
	image="tools.png"
	path="/tools"
/>
...
<main class="mx-auto w-full max-w-[1180px] flex-1 px-4 pt-8 pb-16 sm:px-6 sm:pt-10">
	<div class="grid gap-x-12 gap-y-8 lg:grid-cols-[minmax(0,1fr)_232px]">
		<div class="min-w-0">
			<h1 class="text-[22px] leading-tight font-medium tracking-tight text-ink">All tools</h1>
			...
			>
				...
				{#each data.catalog.categories as category (category.id)}
					<Chip
			...
			<div class="mt-6 flex items-center justify-between border-t border-line pt-3">
				<p class="text-[12.5px] text-faint tabular-nums">
					...
					{#if visible.length !== data.catalog.tools.length}
						<span class="text-faint/70">of {data.catalog.tools.length}</span>
					{/if}
				</p>
				{#if chips > 0}
					<button
						...
						<ul class="grid gap-3 sm:grid-cols-2">
							{#each group.tools as tool, i (tool.id)}
								...
								<ToolCard
									...
									stale={isStale(tool.lastVerified, data.builtAt)}
								/>
							{/each}
						</ul>
					</section>
